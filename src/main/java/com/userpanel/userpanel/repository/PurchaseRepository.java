...
public interface PurchaseRepository extends JpaRepository<Purchase, UUID> {
    ...
    @Query(value = "SELECT p.id as id, p.name as name, p.shop as shop, p.dateTime as dateTime, size(p.items) as itemCount FROM Purchase p WHERE p.account.user.id = :userId")
